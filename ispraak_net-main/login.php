...
//This page will collect a user's email adress and IP and send them an authentication link
//Authentication is confirmed EITHER if the IP and TOKEN match the database, OR if
//the TOKEN and a browser cookie match

//Can this page be simplified with ONE output FORM and TWO message variables
//One for the Banner text and one for the body of the container? 

//perhaps the VERY first thing to do is check for an existing authentication cookie??? 
//then decide whether or not to show the login form

//still some display issues with form that need fixing
//grab all possible query strings, including token and DESTINATION
//so you can include those in the redirect once authenticated
//authentication function can include TOKEN and IP address and SALT hash
...
if ($action == "email")
{
	...
	//and insert $ispraak_time - $ispraak_token - $visitor_ip - $email
	...
	//email
    	...
    	//maybe not send email in plain text
	...
	//send user an authentication link with possible destination if LOGIN variable is good
	
	//email function
	
	//end good email if statement
	}
...
echo "$ispraak_header
		...
		<form id=\"form_1007732\" class=\"ispraak_form\"  method=\"post\" action=\"login.php?action=email\">
					...
					<div class=\"form_description\">
		$ispraak_logo
			Welcome to iSpraak! In order to continue to the requested page, you will need to authenticate your account. Please provide your e-mail address and wait for a login link to arrive.<br><br>
		</div>					
		...
		</div><p class=\"guidelines_on\" id=\"guide_1\">Coming soon! <br><br>Connect with your Google or Microsoft Account!<br></p> 
		</li>		
		
					<li id=\"li_2\" >
		<label class=\"description\" for=\"student_email\">$student_name_field4 </label>
		<div>
			<input id=\"student_email\" type=\"hidden\" name=\"student_email\" $field_status class=\"element text medium\" type=\"text\" maxlength=\"255\" value=\"$start_email\"/> 
...
echo "<li>$key6 | <a href=\"$domain_name/ispraak.php?mykey=$key1&mykey2=$key2\">Student Link</a> | <a href=\"$domain_name/grades.php?mykey=$key1&mykey2=$key2\">Grades</a> | $key5</li>"; 
...
//echo "Here's what I found, $auth_email, $ispraak_token -- $auth_ip ($visitor_ip) AND $auth_time ($ispraak_time)";
